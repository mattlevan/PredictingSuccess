...
def draw(graph, options, physics=False, limit=100):
    ...
    for row in data:
        ...
        source_info = get_vis_info(source, source_id)

        if source_info not in nodes:
            nodes.append(source_info)

        if rel is not None:
            target_info = get_vis_info(target, target_id)

            if target_info not in nodes:
                nodes.append(target_info)

            # edges.append({"from": source_info["id"], "to": target_info["id"], "label": rel.type()})
            edges.append({"to": target_info["id"], "from": source_info["id"],
                "label": rel.type()})
